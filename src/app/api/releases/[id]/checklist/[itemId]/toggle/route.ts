...
export async function PATCH(
  ...
  try {
    ...
    if (!item || item.releaseId !== id) {
      return NextResponse.json(
        { error: 'Checklist item not found for this release' },
        { status: 404 },
      )
    }
    ...
    const timelineEntries = [
      {
        type: updatedItem.completed
          ? 'CHECKLIST_COMPLETED'
          : 'CHECKLIST_UNCHECKED',
        message: updatedItem.completed
          ? `Checklist item completed: ${updatedItem.title}`
          : `Checklist item unchecked: ${updatedItem.title}`,
        releaseId: id,
      },
    ]
    ...
    console.error(
      'PATCH /api/releases/[id]/checklist/[itemId]/toggle error:',
      error,
    )
    return NextResponse.json(
